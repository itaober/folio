...
/**
 * One recent-list row. Default shows favicon · title · domain·date · status
 * pill. Hover/focus swaps the pill for action icon buttons (Save-spot when the
 * row is the active tab's Reading item, then Remove). A brand dot on the
 * favicon marks a saved resume snapshot.
 */
  ...
  return (
    <div className="group/item relative flex items-center gap-2.5 rounded-lg px-3 py-2.5 transition-colors duration-[var(--dur-normal)] ease-[var(--ease)] hover:bg-[var(--muted)] focus-within:bg-[var(--muted)]">
      ...
      >
        ...
        <span className="fz-xs mt-0.5 flex items-center gap-1.5 text-muted-foreground">
      ...
      <div className="flex flex-none items-center gap-0.5">
        {/* Save-spot only appears for the active Reading item, so it can reveal
            on hover without reserving width for every row. */}
        {canSaveProgress ? (
          <IconButton
            size="sm"
